@props(['countries', 'professions', 'experiences', 'educations', 'skills', 'candidateLanguages'])
                        ...
                        <div class="top-content d-flex flex-column flex-lg-row align-items-center leaflet-map-results">
                            <div class="inputbox_1 fromGroup has-icon">
                                <input type="text"
                                    ...
                                    name="keyword" placeholder="{{ __('candidate_title_keyword') }}">
                                <div class="icon-badge">
                                    <x-svg.search-icon stroke="var(--primary-500)" width="24" height="24" />
                                </div>
                            </div>
                            ...
                            @else
                                <div class="inputbox_2 fromGroup has-icon">
                                    ...
                                    <input type="text" id="leaflet_search" placeholder="{{ __('enter_location') }}" name="location" value="{{ request('location') }}" class="tw-border-0" autocomplete="off"/>
                            ...
                            <div class="flex-grow-0 rt-pt-md-20">
                                <button
                                    class="btn btn-primary d-block d-md-inline-block ">{{ __('find_candidate') }}</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="d-flex w-100-p">
                @if (Request::get('keyword'))
                    <div class="rt-mr-2 icon-badge mt-3">
                        <x-website.candidate.filter-data-component title="{{ __('keyword') }}" filter="{{ request('keyword') }}" />
                    </div>
                @endif
                @if (Request::get('country'))
                    <div class="rt-mr-2 icon-badge mt-3">
                        <x-website.candidate.filter-data-component title="{{ __('country') }}" filter="{{ request('country') }}" />
                    </div>
                @endif
                @if (Request::get('sortby') && Request::get('sortby') != 'latest')
                    <div class="rt-mr-2 icon-badge mt-3">
                        <x-website.candidate.filter-data-component title="{{ __('sortby') }}" filter="{{ request('sortby') }}" />
                    </div>
                @endif
                ...
                @if (Request::get('gender') && Request::get('gender') != 'all')
                    <div class="rt-mr-2 icon-badge mt-3">
                        <x-website.candidate.filter-data-component title="{{ __('gender') }}" filter="{{ request('gender') }}" />
                    </div>
                @endif
                            ...
                            <div class="flex-grow-0 rt-mb-24">
                                <div class="joblist-fliter-gorup">
                                    <div class="left-content">
                                        <select onchange="$('#form').submit();" name="status"  class="rt-selectactive gap w-100-p">
                                            <option {{ request('status') ? '' : 'selected' }} value="">
                                                {{ __('all_status') }}
                                            </option>
                                            <option {{ request('status') == 'available' ? 'selected' : '' }}
                                                value="available">
                                                {{ __('available') }}
                                            </option>
                                        ...
                                        <select name="language" id="perpage" class="rt-selectactive gap w-50">
                                            <option value="">{{ __('all_language') }}</option>
                                            @foreach ($candidateLanguages as $language)
                                                <option {{ request('language') == $language->id ? 'selected':''}} value="{{ $language->id }}">{{ $language->name }}</option>
                                            @endforeach
                                        </select>
                                        <select name="sortby" id="sortby" class="rt-selectactive w-100-p">
                                            ...
                                            <div class="benefits-tags">
                                                @foreach ($skills as $skill)
                                                    <label for="{{ $skill->name }}" class="py-1">
                                                        <input onclick="Filter()" {{ request('skills') ? (in_array($skill->id, request('skills')) ? 'checked':''):'' }} type="checkbox" id="{{ $skill->name }}" value="{{ $skill->id }}" name="skills[]">
