<template>
    <div>
        <div class="mt-5 application-wrapper-bottom position-relative">
            ...
            <div class="all-application-column column mb-3" v-for="applicationGroup in applicationsGroupData"
                ...
                <div class="height-auto">
                    ...
                    <draggable class="list-group kanban-column" v-model="applicationGroup.applications"
                        ...
                        <div class="application-card" v-for="application in applicationGroup.applications"
                            :key="application.id">
                            <div class="appliaction-card-top" data-toggle="modal" @click.prevent="previewResume(application.candidate.user.username)">
                                <div class="profile-img" v-if="application.candidate.user">
                                    <img width="48px" height="48px" class="tw-object-cover	" :src="application.candidate.user.image_url"
                                        alt="image">
                                </div>
                                ...
                                <form @submit.prevent="updateGroup">
                                    ...
                                    <div class="d-flex justify-content-between mt-3 mb-5">
                                        <button type="button" class="btn btn-secondary"
                                            @click="showModal = false">{{ __('cancel') }}</button>
        ...
        <!-- Profile modal  -->
        <CandidateResumeModal :response="candidateResumeInformation" :show="showCandidateResumeModal" @close-modal="showCandidateResumeModal = false" v-if="showCandidateResumeModal" :language="languageTranslation"/>
    </div>
</template>
